-- Script CORREGIDO para probar que las encuestas activas se muestren correctamente
...
-- DELETE FROM surveys WHERE id IN (SELECT id FROM surveys WHERE question LIKE '%TEST%');
...
-- 4. Obtener los IDs reales de las opciones creadas
SET @option1_id = (SELECT id FROM survey_options WHERE survey_id = @survey_id AND option_text LIKE '%cuidado facial%' LIMIT 1);
SET @option2_id = (SELECT id FROM survey_options WHERE survey_id = @survey_id AND option_text LIKE '%maquillaje%' LIMIT 1);
SET @option3_id = (SELECT id FROM survey_options WHERE survey_id = @survey_id AND option_text LIKE '%cuidado corporal%' LIMIT 1);

-- 5. Verificar que la encuesta está en draft (no visible para clientes)
SELECT 
...
-- 6. Verificar que las opciones están aprobadas
SELECT 
...
-- 7. Cambiar estado a active para hacerla visible
UPDATE surveys SET status = 'active' WHERE id = @survey_id;

-- 8. Verificar que ahora está activa
SELECT 
...
-- 9. Simular algunos votos para probar la funcionalidad (usando IDs reales)
INSERT INTO survey_votes (survey_id, option_id, user_id) VALUES 
(@survey_id, @option1_id, 1),      -- Usuario 1 vota por opción 1
(@survey_id, @option2_id, 2),      -- Usuario 2 vota por opción 2
(@survey_id, @option3_id, 3);      -- Usuario 3 vota por opción 3

-- 10. Verificar votos registrados
SELECT 
...
-- 11. Verificar que la encuesta aparece en la consulta de encuestas activas
SELECT 
...
-- 12. Verificar opciones con conteo de votos
SELECT 
...
-- 14. Limpiar datos de prueba (descomentar para limpiar)
